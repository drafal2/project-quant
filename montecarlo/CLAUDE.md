...
- **`uniform/`** — six concrete `Sampler` implementations. PRNGs: `KnuthSampler` (Knuth RANARRAY lagged-Fibonacci in IEEE doubles, KK=100/LL=37/TT=70 — **bit-for-bit compatible with `ql.KnuthUniformRng`** for any non-zero seed), `MRG32k3aSampler` (L'Ecuyer 1999 combined MRG, period ~2^191, production default for serial paths; `substream(index)` stub raises `NotImplementedError` until the path-engine PR), `LecuyerLCG1988Sampler` (L'Ecuyer 1988 combined-LCG + Bays-Durham shuffle, period ~2^61, **bit-for-bit compatible with `ql.LecuyerUniformRng`** — included only for cross-validation; use `MRG32k3aSampler` in production), `MersenneTwisterSampler` (canonical MT19937 with vectorised tempering and a Python-loop refill, bit-for-bit compatible with `ql.MersenneTwisterUniformRng`; also exposes `next_int32_block(n)` for raw tempered uint32 output). QMC: `HaltonSampler` (prime-base van der Corput; supports both the deterministic origin-skip mode and a `random_start=True` mode that draws per-dimension start indices from MT19937 — the latter is bit-for-bit compatible with `ql.HaltonRsg(d, seed, randomStart=True)` for any non-zero seed), `SobolSampler` (gray-code Sobol with Joe-Kuo (2008) direction numbers covering dimensions 1-1024, table in `_joe_kuo_data.py`, matches `ql.SobolRsg(JoeKuoD6)` up to a constant 2^-33 open-interval ULP shift).
