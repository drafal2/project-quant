...
- **Local volatility (Dupire)** — drop into `volatility/local_vol.py` as `DupireLocalVol(surface)`. Dispatches on `isinstance(surface, DifferentiableVolSurface)` for the analytical path, falls back to a smoothed (T, S) grid for plain `VolSurface`.
