...
- `markdown>=3.7` added to the `[validation]` extra, used by `validation/render_html.py`
- `validation/render_html.py` — re-runs the cross-validation suite, captures pass/fail and the per-pair pricing diagnostics, then renders `validation/REPORT.md` to `validation/REPORT.html` with the run banner pinned at the top. Run via `.venv/Scripts/python validation/render_html.py`; HTML is self-contained (embedded CSS, no external resources)
- `validation/REPORT.html` — browser-friendly snapshot of the cross-validation report, regenerated by the renderer above
