...
| `montecarlo/` | Random-number sampling (PRNGs, QMC, `U -> N` transforms, diagnostics), diffusion-side volatility models (`ConstantVol`, `BlackTermStructureVol`, `DupireLocalVol`), and path generation (`TimeGrid`, `EulerLogPathEngine`) | [`montecarlo/CLAUDE.md`](montecarlo/CLAUDE.md) |
